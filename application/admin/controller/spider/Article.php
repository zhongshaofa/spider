...
class Article extends AdminController {
    ...
    public function __construct() {
        parent::__construct();
        $this->model = new \app\admin\model\spider\Article();
    }
...
//        }
    ...
    public function status() {
        ...
        //验证数据
//        $validate = $this->validate($get, 'app\admin\validate\spider\Article.status');
//        if (true !== $validate) return __error($validate);

        //判断状态
        $status = $this->model->where('id', $get['id'])->value('status');
        list($msg, $status) = $status == 1 ? ['禁用成功', $status = 0] : ['启用成功', $status = 1];
